...
class _ShoppingCartPageState extends State<ShoppingCartPage> {
  ...
  late Map<ProductAd, int> productAdQuantities; // ProductAd -> qty
  Store? store;
  bool multipleStoresDetected = false;

  // Controlador para o AnimatedList
  final GlobalKey<AnimatedListState> _listKey = GlobalKey<AnimatedListState>();

  // Lista para manter a ordem dos produtos para o AnimatedList
  ...
  void _loadCartProducts() {
    ...
    for (var map in widget.cart.productsQty ?? []) {
      for (var entry in map.entries) {
        ...
        if (detectedStore == null) {
          ...
          // Produtos de bancas diferentes -> marcar como múltiplas lojas detectadas
          multipleStoresDetected = true;
          // Não adiciona este produto
          continue;
        }
    ...
    // Atualiza a lista ordenada para o AnimatedList
  ...
  void _updateCartFromState() {
    ...
    widget.cart.productsQty = updatedList;
  }
                  ...
                  Row(
                    children: [
                      IconButton(
                        ...
                        onPressed: () => _changeQuantity(productAd, -1),
                      ),
